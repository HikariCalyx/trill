...
settings-streamer-mode = 直播隱私模式
    .tooltip = 啓用此模式將為保存查看器添加一個額外的「封面」選項卡，該選項卡隱藏有關當前保存文件的所有信息。
settings-debug = 顯示調試信息
settings-input-delay = 輸入延遲
settings-input-socd-resolution = 對向輸入 (SOCD) 清理
    .natural = 不清理
    .neutral = 全回中 (SOCD-N)
    .last-input-priority = 後指令覆蓋 (SOCD-L)
...
settings-matchmaking-usecnserver = 使用 Trill 中國大陸伺服 (區域鎖定)
settings-enable-patch-autoupdate = 自動更新
settings-quick-initialize = 快速初始化
